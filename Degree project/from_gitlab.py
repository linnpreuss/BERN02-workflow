...
# -*- coding: utf-8 -*-
"""
Created on Thu Jan 29 00:38:17 2026
...
class CrankNicolsonSolver(SchrodingerSolver):
    def __init__(self, parameters=None, useSparse=True):  # Default to sparse
        ...
        self.solveA = factorized(A)  # LU factorization (fast)
        self.B = B
        
        # Don't create dense matrices at all!
        self.M = None  # We won't use this for sparse mode

    def step(self):
        if self.t >= self.p.T:
            return False

        # Always use sparse solver
        rhs = self.B @ self.psi
